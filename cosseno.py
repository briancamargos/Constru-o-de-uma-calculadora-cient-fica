...
def Cosseno(graus:float): # a função será montada presupondo que o angulo "a" de entrada está em graus.
    ...
    aux2 = 0                  #usado na formula da série de taylor
    while (termo_n >= 0.001): #Faz o calculo até um dos termos n da série ser menor que 0.001
        ...
        if (aux1 == 1):
            cosseno = 1                        #o primeiro termo
        elif (aux1 %2 == 0):
            cosseno = cosseno - termo_n        #o somatorio dos termos n pares
        else:
            cosseno = cosseno + termo_n        #o somatorio dos termos n impares
        
        aux1 += 1
        aux2 += 2
    
    cosseno = round(cosseno,4)                 #arrendondamento para 4 casas decimais
    return cosseno